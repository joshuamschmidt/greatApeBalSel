...
setwd("/Users/joshuaschmidt/Projects/ncd_scores/bedGraph")
...
setwd("/Users/joshuaschmidt/Projects/ncd_scores/wig")
# wig tracks...
for (sp in speciesNCDTablesDT[,unique(species)]) {
    col <- colours[sp]
        outfile <- paste(sp,"_ncd.log10p_ucsc.track.bedGraph",sep="")
...
## bash
cd /Users/joshuaschmidt/Projects/ncd_scores/bedGraph
for f in $(ls *.bedGraph | sed 's/\.bedGraph//g');
do sort -k1,1 -k2,2n ${f}.bedGraph |awk '$3-$2==101' > ${f}.bedGraph.sorted;
../bin/bedGraphToBigWig ${f}.bedGraph.sorted hg19.chrom.sizes ${f}.bw; rm ${f}.bedGraph.sorted; 
rm ${f}.bedGraph;
done
